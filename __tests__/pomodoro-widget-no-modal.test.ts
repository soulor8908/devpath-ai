...
// 守护测试：PomodoroWidget 不得使用 large Modal，必须用 ring/card/hidden/expanded 四态浮窗
//
...
//   2. 不渲染 <Modal> JSX 元素
//   3. WidgetMode 类型必须为四态 "hidden" | "ring" | "card" | "expanded"（不接受 "small"/"large"）
...
describe("PomodoroWidget：移除 large Modal，四态浮窗", () => {
  ...
  it("WidgetMode 类型必须为四态 hidden/ring/card/expanded", () => {
    // 匹配 type WidgetMode = "hidden" | "ring" | "card" | "expanded";
    // 顺序允许调整（防止 reorder 导致测试误判）
    const modeRegex = /type\s+WidgetMode\s*=\s*"hidden"\s*\|\s*"ring"\s*\|\s*"card"\s*\|\s*"expanded"/;
  ...
  it("expanded 浮窗用 ExpandedWidget 组件承载（2026-07-25 新增）", () => {
